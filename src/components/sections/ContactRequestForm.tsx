...
export function ContactRequestForm({ dictionary }: { dictionary: Dictionary }) {
    ...
    <form className="grid gap-5" onSubmit={handleSubmit}>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
        <button
          ...
          type="submit"
        >
          {disabled ? dictionary.contactRequest.submitting : dictionary.common.submit}
        </button>
        {status === "success" ? (
          <p className="text-sm leading-6 text-olive">{dictionary.contactRequest.success}</p>
        ) : null}
        {status === "error" ? (
          <p className="text-sm leading-6 text-red-900">{dictionary.contactRequest.error}</p>
        ) : null}
      </div>
    </form>
  );
